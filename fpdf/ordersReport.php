   ...
   // Cabecera de página
    function Header()
    {
      ...
      $this->SetDrawColor(163, 163, 163); //colorBorde
        ...
        $this->Cell(30, 10, utf8_decode('Nro de pedido'), 1, 0, 'C', 1);
        $this->Cell(60, 10, utf8_decode('Cliente'), 1, 0, 'C', 1);
        $this->Cell(30, 10, utf8_decode('Cedula o Rif'), 1, 0, 'C', 1);
        $this->Cell(30, 10, utf8_decode('Total'), 1, 0, 'C', 1);
        $this->Cell(80, 10, utf8_decode('Fecha'), 1, 1, 'C', 1);
    }
...
$query = $conn->query('SELECT * FROM orders JOIN clients WHERE id_client = clients.idC');

while ($data = $query->fetch_object()) {     
   /* TABLA */
    $pdf->Cell(30, 10, utf8_decode($i), 1, 0, 'C', 0);
    $pdf->Cell(30, 10, utf8_decode($data->id), 1, 0, 'C', 0);
    $pdf->Cell(60, 10, utf8_decode($data->nameC . " " . $data->lastNameC), 1, 0, 'C', 0);
    $pdf->Cell(30, 10, utf8_decode($data->dniC), 1, 0, 'C', 0);
    $pdf->Cell(30, 10, utf8_decode($data->total), 1, 0, 'C', 0);
    $pdf->Cell(80, 10, utf8_decode($data->date), 1, 1, 'C', 0);
    $i++;
    }
